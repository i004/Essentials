...
export function progressbar (percent: number, width: number = 5) {
    const emojis = [
    ...
    return `${fill < 1 ? emojis[0] : emojis[1]}${emojis[3].repeat(fill)}${emojis[2].repeat(empty)}${fill >= width ? emojis[5] : emojis[4]}`;
}
