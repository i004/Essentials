import { ApplicationCommand, ApplicationCommandManager, ChatInputApplicationCommandData, Snowflake } from 'discord.js';
...
export class CommandManager {
  ...
  public async registerCommands (commands: object[]) {
    ...
    const build = commands.map(x => ({ data: this.build(x), run: new (x as any)().run, proto: x }));
    ...
    for (const cId in commandCache) {
      ...
      if (!command) {
        ...
        commandCache[cId] = (await this.commands.edit(cId, command.data, command.data.guildId)).toJSON();
